...
/// Column order of templates/ADV-profiles.xlsx.
///
/// Deliberately independent of the campaign mode: the workbook is generated by
/// tools/make_template.py, which cannot know the mode, and the two must stay in
/// step. In field mode the TKE column therefore keeps its laboratory heading;
/// fillProfileTemplate() notes the caveat in the sheet instead. The per-point
/// export in writePointStats() builds its own headers and does relabel.
                     ...
                     QString *errorString)
{
    ...
    const QStringList headers = {
        ...
        QStringLiteral("u'v' (m^2/s^2)"), QStringLiteral("u'w' (m^2/s^2)"),
        QStringLiteral("v'w' (m^2/s^2)"),
        labels::tkeColumn(model.mode()), QStringLiteral("eps (m^2/s^3)"),
        QStringLiteral("U magnitude (m/s)"), QStringLiteral("direction (deg)"),
    ...
    for (const MeasurementPoint &p : model.points()) {
        ...
        if (labels::epsEstimable(s.u.n, series->samplingFrequency))
            writeNumber(doc, row, col++, s.eps);
        else
            doc.write(row, col++, labels::epsText(s.eps, s.u.n, series->samplingFrequency));
                         ...
                         const QString &outputPath, QString *errorString)
{
    ...
    if (model.mode() == Mode::Field) {
        // the headings come from the fixed template, so the caveat goes beside
        // the title rather than into the TKE column name
        doc.write(1, 3,
                  QStringLiteral("Field mode: TKE is a variance proxy (low sampling rate, "
                                 "no inertial subrange resolved) and eps is not estimable."));
    }
